...
class STLExporter(private val context: Context) {
    ...
    fun exportWithConfiguration(
        model: ClayModel,
        filename: String,
        config: uk.org.retallack.claymodeler.export.ExportConfiguration
    ): String {
        val finalModel = if (config.attachmentType != uk.org.retallack.claymodeler.export.AttachmentType.NONE) {
            val attachment = when (config.attachmentType) {
                uk.org.retallack.claymodeler.export.AttachmentType.BASE ->
                    uk.org.retallack.claymodeler.export.geometry.BaseGenerator().generate(model, config.baseConfig, config.sizeInMm)
                uk.org.retallack.claymodeler.export.AttachmentType.KEYRING_LOOP ->
                    if (config.placement != null) uk.org.retallack.claymodeler.export.geometry.LoopGenerator().generate(model, config.keyringConfig, config.placement!!, config.sizeInMm) else null
                uk.org.retallack.claymodeler.export.AttachmentType.WALL_HOOK ->
                    if (config.placement != null) uk.org.retallack.claymodeler.export.geometry.HookGenerator().generate(model, config.hookConfig, config.placement!!, config.sizeInMm) else null
                uk.org.retallack.claymodeler.export.AttachmentType.NONE -> null
            }
            if (attachment != null) {
                val merged = uk.org.retallack.claymodeler.export.geometry.GeometryMerger().merge(model, attachment)
                val manifold = uk.org.retallack.claymodeler.export.geometry.GeometryMerger().validateManifold(merged)
                if (!manifold.valid) {
