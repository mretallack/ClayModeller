package uk.org.retallack.claymodeler.ui.wizard

import androidx.lifecycle.LiveData
import androidx.lifecycle.MutableLiveData
import androidx.lifecycle.ViewModel
import uk.org.retallack.claymodeler.export.AttachmentType
import uk.org.retallack.claymodeler.export.BaseConfig
import uk.org.retallack.claymodeler.export.ExportConfiguration
import uk.org.retallack.claymodeler.export.HookConfig
import uk.org.retallack.claymodeler.export.KeyringConfig
import uk.org.retallack.claymodeler.export.PlacementResult
import uk.org.retallack.claymodeler.export.placement.PlacementController
...
class ExportWizardViewModel : ViewModel() {
    ...
    fun rotateModel90(axis: Char) {
        ...
        for (i in model.vertices.indices) {
            ...
            model.vertices[i] = when (axis) {
                'x' -> uk.org.retallack.claymodeler.model.Vector3(v.x, -v.z, v.y)
                'y' -> uk.org.retallack.claymodeler.model.Vector3(v.z, v.y, -v.x)
                'z' -> uk.org.retallack.claymodeler.model.Vector3(-v.y, v.x, v.z)
                else -> v
            }
        }
    ...
    fun rotateModelSmooth(angleX: Float, angleY: Float) {
        ...
        for (i in model.vertices.indices) {
            ...
            v = uk.org.retallack.claymodeler.model.Vector3(v.x, v.y * cosX - v.z * sinX, v.y * sinX + v.z * cosX)
            v = uk.org.retallack.claymodeler.model.Vector3(v.x * cosY + v.z * sinY, v.y, -v.x * sinY + v.z * cosY)
            model.vertices[i] = v
        }
        model.calculateNormals()
        _model.value = model
    ...
    fun bakeModelMatrix(matrix: FloatArray) {
        ...
        for (i in model.vertices.indices) {
            ...
            model.vertices[i] = uk.org.retallack.claymodeler.model.Vector3(out[0], out[1], out[2])
        }
        model.calculateNormals()
        _model.value = model
    }
